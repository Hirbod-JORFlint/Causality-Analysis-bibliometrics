...
def investigate_cooccurrence(filename, keyword_column, separator, A_num):
    ...
    data = pd.DataFrame(index=[edge for edge, _ in co_occurrences])
    for edge in edges:
        # Create a binary column for each edge indicating whether the corresponding keywords co-occur in each article
        data.loc[[edge], 'co_occurrence'] = df[keyword_column].apply(lambda keywords: 1 if isinstance(keywords, tuple) and set(edge).issubset(keywords) else 0)
    data.to_csv('data.csv')

    # Define state names for each variable
    unique_keywords = set(itertools.chain.from_iterable(df[keyword_column].dropna()))
    state_names = {keyword: ['0', '1'] for keyword in unique_keywords}
...
model, co_occurrences = investigate_cooccurrence("math.csv", "Keywords", ";", 2)
