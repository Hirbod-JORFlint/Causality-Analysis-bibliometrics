...
# Perform ADF test again on the transformed non-stationary columns
non_non_stationary_columns = []
for column in non_stationary_columns:
    is_stationary = perform_adf_test(df_new[column])
    if not is_stationary:
        # Add the column to the 'non-non stationary' list
        non_non_stationary_columns.append(column)
        # Drop the column from the dataframe
        df_new = df_new.drop(columns=[column])

# Print the list of 'non-non stationary' columns
print("\n'Non-non stationary' columns:", non_non_stationary_columns)
