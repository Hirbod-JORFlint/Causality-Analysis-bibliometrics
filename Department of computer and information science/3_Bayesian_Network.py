...
# select first 50
def investigate_cooccurrence(filename, keyword_column, separator, A_num, nrows=100):
...
# or select 50 rows randomly
def investigate_cooccurrence(filename, keyword_column, separator, A_num, random_state=None):
    df = pd.read_csv(filename)

    # Randomly select rows
    df = df.sample(n=90, random_state=random_state)
